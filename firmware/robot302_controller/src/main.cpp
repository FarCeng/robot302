...
void cmdVelCb(const void * msgin)
{
  ...
  const float wheel_radius = 0.106458f;
  ...
  if (now - prev_ms >= 50) {
    ...
    if (TEST_MODE == 0) {
      ...
      if (fabsf(smoothed_target_L) < 1.0f && fabsf(smoothed_target_R) < 1.0f) {
        ...
        applied_pwm_R = (int)((float)applied_pwm_R * 0.87f);
        ...
        // D. IMU HEADING HOLD (Error Correction 2)
        if (fabsf(cmd_angular_z) < 0.05f && (fabsf(smoothed_target_L) > 1.0f || fabsf(smoothed_target_R) > 1.0f)) {
          if (!is_yaw_locked) {
          ...
          float Kp_yaw = 65.0f; 
          int yaw_correction = (int)(yaw_error * Kp_yaw);
